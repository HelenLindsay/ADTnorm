...
#' valley locations. Each panel is an ADT marker and each track is a sample.
#' Color by batch
...
#' about. Leave it NULL will generate figures for all the ADT markers available
#' in cell_x_adt.
...
plot_adt_density_with_peak_valley = function(cell_x_adt, cell_x_feature,
    ...
    resPlot = ggplot(tmpProfile, aes(x = counts, y = sample)) +
        ...
        geom_segment(data = peak_location[[1]],
                     aes(x = peakx, xend = peakx,
                         y = peaks, yend = peaky + peaks), linewidth = 1) +
        geom_segment(data = valley_location[[1]],
                     aes(x = peakx, xend = peakx,
    ...
    if (ncol(peak_landmark_list) >= 2) {
        resPlot = resPlot +
            geom_segment(data = peak_location[[2]],
                         aes(x = peakx, xend = peakx, y = peaks,
                             yend = peaky + peaks), size = 1)
    }

    if (ncol(peak_landmark_list) >= 3) {
        resPlot = resPlot +
            geom_segment(data = peak_location[[3]],
                         aes(x = peakx, xend = peakx, y = peaks,
                             yend = peaky + peaks), linewidth = 1) +
            geom_segment(data = valley_location[[2]],
                         aes(x = peakx, xend = peakx, y = peaks,
                             yend = peaky + peaks), linewidth = 1, color = "grey")
    }
    if (ncol(peak_landmark_list) >= 4) {
        resPlot = resPlot +
            geom_segment(data = peak_location[[4]],
                         aes(x = peakx, xend = peakx,
                             y = peaks, yend = peaky + peaks), linewidth = 1) +
            geom_segment(data = valley_location[[3]],
                         aes(x = peakx, xend = peakx, y = peaks,
                             yend = peaky + peaks), linewidth = 1, color = "grey")
    }
