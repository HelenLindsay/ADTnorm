...
#' @param peak_mode_res The peak landmark results coming out of `get_peak_mode`
#' or `get_peak_midpoint` function.
...
#' @param multi_sample_per_batch Set it to TRUE to discard the positive peak
#' that only appear in one sample per batch (sample number is >=3 per batch).
#' @param adjust Parameter for `density` function: bandwidth used is actually
...
#' asinh(8/5+1) if the right 95% quantile of IgG samples are large.
...
get_valley_location = function(cell_x_adt, cell_x_feature, adt_marker_select,
    ...
    ## tag sample if it is the only sample within each batch that have
...
# .filterPositivePeaks ----
.filterPositivePeaks <- function(cell_x_feature, peak_landmark_list, tag_row,
                                 min_samples=3){
    ## If there are multiple samples per batch,
    ## remove the positive peaks that only appear in one sample
    for (batch_each in unique(cell_x_feature$batch)){
        sample_index = cell_x_feature %>%
            dplyr::filter(batch == batch_each) %$% sample %>% unique

        ## if there is only one second peak, remove it
        if (length(sample_index) >= min_samples){
            ## which col has only 1 peak per batch
            col_index = which(colSums(!is.na(peak_landmark_list[sample_index, ])) == 1)
            ## across those columns that only have 1 peak per batch
            for (c in col_index){
                ## get the sample name
                row_index = sample_index[which(!is.na(peak_landmark_list[sample_index, c]))]
                # common_peak_n #non-zero value, record the peak number that is
                # common across samples in this batch
                tag_row[[row_index]] = c(tag_row[[row_index]], c)
            }
        }
    }
    return(tag_row)
