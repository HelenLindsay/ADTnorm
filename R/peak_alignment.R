...
#' (columns) format.
#' @param cell_x_feature Matrix of cells (rows) by cell features (columns)
#' such as cell type, sample, and batch related information.
#' @param landmark_matrix Matrix of peak and valley landmarks after fill in
#' NA using `landmark_fill_na` function.
#' @param target_landmark Leave it as NULL to align the landmark to the mean
#' location across samples. Denote it by a vector of the same length of the
#' column number of landmark to align the negative peak, valley and positive
#' peak(s) to the specified fixed location.
#' @export
#' @examples
#' \dontrun{
...
# require(flowStats)
# require(fda)
peak_alignment = function(cell_x_adt, cell_x_feature=NULL, landmark_matrix=NULL,
    ...
    for (sample in samples){
        ...
        if (length(cell_ind) > 0){
          density_y = cbind(density_y,
                            stats::density(cell_x_adt[cell_ind],
                                           from=from, to=to, n=nb, na.rm=TRUE)$y)
        } else{
    ...
    return(list(cell_x_adt_norm=cell_x_adt_norm,
                landmark_matrix_norm=warped_landmark_matrix))
}
